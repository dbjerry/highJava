...
public class MemberDao implements IMemberDao {
	...
	public Object insertMember(MemberVO memVo) {
		...
		try {
			obj = smc.insert("login_member.insertMember", memVo);
	...
	public int deleteMember(String memId) {
		...
		int cnt = 0;
		try {
			cnt = smc.delete("login_member.deleteMember", memId);
		} catch (SQLException e) {
			cnt = 0;
			e.printStackTrace();
		} 
	...
	public int updateMember(MemberVO memVo) {
		...
		try {
			...
			cnt = smc.update("login_member.modifyMember", memVo);
		} catch (SQLException e) {
			cnt = 0;
			e.printStackTrace();
		} 
	...
	public List<MemberVO> getMemberList() {
		...
		try {
			memList = smc.queryForList("login_member.selectAllMember");
		} catch (SQLException e) {
			memList = null;
			e.printStackTrace();
		} 
	...
	public int getMember(String memId) {
		...
		try {
			count = 
				(int)smc.queryForObject("login_member.getMemberCount", memId);
		} catch (SQLException e) {
			count = 0;
			e.printStackTrace();
		} 
	...
	public int getMemberPw(String memPw) {
		...
		try {
			count = 
				(int)smc.queryForObject("login_member.getMemberCountPw", memPw);
		} catch (SQLException e) {
			count = 0;
			e.printStackTrace();
		} 
...
//		} 
	...
	public List<MemberVO> searchMember(Map<String, String> param) {
		...
		try {
			memList.add((MemberVO)smc.queryForObject("login_member.getFindMember", param));
			System.out.println(memList.get(0).getMem_name());
		} catch (SQLException e) {
			memList = null;
			e.printStackTrace();
		} 
